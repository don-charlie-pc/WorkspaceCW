...
* 
* ---INIT process---
* First, this driver waits 20 milliseconds for LCD booting. After that, it changes to 4 bit mode.
* Then, it writes the required commands for LCD configuration, and finally changes to LCD IDLE state.
* 
* ---DELAYS---
* This module uses three types of delays performed with asm instructions:
* -> 20 ms: LCD booting.
...
#define LCD_DATA_BITS_MASK 0xD0
#define D4_PIN 4
#define D5_PIN 5
#define D6_PIN 6
#define D7_PIN 7
...
{		
	...
	GPIO_ConfigPinDirection(PORTA,D5_PIN,OUTPUT);
	...
	{	
		switch(sgu8currentState)
		{
			...
			case WRITE_CHAR1:
				...
				if(sgu8counter1==0)
				{
					sgu8currentState = SET_POSITION2;
				}
				break;
			case SET_POSITION2:
				sgu8currentState = WRITE_CHAR2;
				//Update cursor position for line 2
				LcdManager_SetRequestedPosition(2);
				break;
			case WRITE_CHAR2:
				LcdManager_Send(*sgpu8String2,RS_DATA);
				sgpu8String2++;
				sgu8counter2--;
				//Write one character for each call to this function
				if(sgu8counter2==0)
